...
- **Scores below 6 are outside the formula's range.** The formula was designed for adult prose at sixth-grade level and above. Using it to assess simple text for children is not appropriate — use [Spache](/metrics/spache/) for grades 1–3 or [Dale-Chall](/metrics/dale-chall/) for grades 4 and above.
...
- [Flesch Reading Ease](/metrics/flesch/) — also uses sentence length and syllables per word, but averages syllables rather than counting complex words
- [SMOG Index](/metrics/smog/) — also counts polysyllabic words, but uses a fixed 30-sentence sample and targets 100% comprehension; preferred for health materials
- [Flesch-Kincaid Grade Level](/metrics/flesch-kincaid/) — grade level output using average syllables per word rather than a polysyllabic word threshold
